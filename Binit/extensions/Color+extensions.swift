//
//  ColorUtils.swift
//  Binit
//
//  Created by Nikita on 25.09.2023.
//

import Foundation
import SwiftUI

extension Color {
    static let mainColor = Color(red: 0.19, green: 0.19, blue: 0.2)
    static let orangeColor = Color(red: 0.98, green: 0.34, blue: 0.03)
    static let searchTextColor = Color(red: 0.31, green: 0.35, blue: 0.41)
    static let searchBodyColor = Color(red: 0.44, green: 0.46, blue: 0.62)
    static let searchBubbleTextColor = Color(red: 0.31, green: 0.35, blue: 0.41)
    static let mainGarbageTypeBackColor = Color(red: 0.95, green: 0.95, blue: 0.96)
    static let mainArticleSubTitleColor = Color(red: 0.42, green: 0.44, blue: 0.45)
    static let additionalTextColor = Color(red: 0.31, green: 0.35, blue: 0.41)
    
    static let correctColor = Color(red: 0.17, green: 0.65, blue: 0.31)
    static let wrongColor = Color(red: 0.88, green: 0.18, blue: 0.18)
        

}
